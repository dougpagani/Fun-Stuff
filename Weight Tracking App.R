...
addweight <- function(weight){
  ...
  #caching input
  odata <- read.csv("C:\\Users\\Dean M\\Desktop\\R Programming\\Fun\\Weight Tracking.csv",
                    header = TRUE)
  x <- length(odata$Date) + 1
  odata[x,1] <- as.character.Date(Sys.Date())
  odata[x,2] <- weight
  odata <- subset(odata, is.na(odata$Weight) == FALSE)
  write.csv(odata, file = "C:\\Users\\Dean M\\Desktop\\R Programming\\Fun\\Weight Tracking.csv", 
            row.names = FALSE)
  
  #graphing complete history
  library(ggplot2)
  gdata <- read.csv("C:\\Users\\Dean M\\Desktop\\R Programming\\Fun\\Weight Tracking.csv",
                    header = TRUE)
